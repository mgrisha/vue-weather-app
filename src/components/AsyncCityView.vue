  ...
  <div class="flex flex-col flex-1 items-center">
    ...
    <div class="max-w-screen-md w-full py-12">
      <div class="mx-8 text-white">
        ...
        <div
          v-for="dayInfo in weatherData.forecast.forecastday"
          :key="dayInfo.date_epoch"
          class="flex items-center"
        >
          <p class="flex-1">
            {{ new Date(dayInfo.date).toLocaleDateString('en-us', { weekday: 'long' }) }}
          </p>
          <img class="w-[50px] h-[50px] object-cover" :src="dayInfo.day.condition.icon" alt="" />
          <div class="flex gap-2 flex-1 justify-end">
            <p>H: {{ Math.round(dayInfo.day.maxtemp_c) }}&deg;</p>
            <p>L: {{ Math.round(dayInfo.day.mintemp_c) }}&deg;</p>
          </div>
        </div>
      </div>
    </div>
